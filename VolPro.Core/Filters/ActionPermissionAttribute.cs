...
namespace VolPro.Core.Filters
{
    public class ActionPermissionAttribute : TypeFilterAttribute
    {
       ...
       : base(typeof(ActionPermissionFilter))
        ...
        {
            Array array = Enum.GetValues(typeof(ActionPermissionOptions));
            List<string> actionArray = new List<string>();
            foreach (ActionPermissionOptions item in array)
            {
                if (tableAction.HasFlag(item))
                {
                    actionArray.Add(item.ToString());
                }
            }

            Arguments = new object[] { new ActionPermissionRequirement() {
                 SysController=sysController,
                 TableAction=actionArray.ToArray(),
                 TableName=tableName,
                 IsApi = isApi,
                 RoleIds=roleId
            } };
